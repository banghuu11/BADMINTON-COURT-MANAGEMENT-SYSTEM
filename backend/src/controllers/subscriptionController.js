...
const subscribePlan = async (req, res) => {
  ...
  try {
    ...
    const currentSubResult = await client.query(
      "SELECT EndDate FROM OwnerSubscription WHERE OwnerId = $1 AND Status = 'Active' ORDER BY EndDate DESC LIMIT 1",
      [ownerId]
    );

    if (currentSubResult.rows.length > 0 && new Date(currentSubResult.rows[0].enddate) > new Date()) {
      // Nếu gói cũ vẫn còn hạn, thì StartDate sẽ là sau khi gói cũ hết hạn
      startDate = new Date(currentSubResult.rows[0].enddate);
      endDate = new Date(currentSubResult.rows[0].enddate);
    }
    ...
    // 3. Hủy kích hoạt gói cũ (nếu mua gói mới khác loại, giả định ở đây là cho phép mua đè hoặc gia hạn)
